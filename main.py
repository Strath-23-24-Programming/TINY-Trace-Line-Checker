class Trace:
	def __init__(self):
		self.memory = [0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0] # TINY Memory (16 Bit)
		self.registry = [0x0,0x0,0x0,0x0] # TINY Registry (4 Bit)
		self.inQueue, self.outQueue, self.output = [], [], []
		self.action = ['', '', '', '']
		self.stop = False
		self.reason = ''
	...
	def printState(self) -> None:
		...
		for reg in self.registry: printStr += f'{self.hexToStr(reg)} '  

		printStr += ' '

		for mem in self.memory: printStr += self.hexToStr(mem)

		printStr += ' '
	
		for action in self.action: printStr += f' {action}'
		
		self.output.append(printStr)
...
def makeInputLine(mem, actions:str) -> str:
    reg, out = '', ''

    for pos in mem[0:4]: reg += pos + ' '
    
    mem = mem[4:]
    
    for pos in reg.split(' '): out += str(pos) + ' '
    for pos in mem.split(' '): out += str(pos)
        
    out += ' '
    
    for act in actions.split(' '): out += ' ' + act

    return out

def checkTraceOutput(output:list, traceInput:str) -> str:        
    regLineCheck, memLineCheck, actLineCheck = False, False, False

    for line in output:
        if line.split('  ')[0] == traceInput.split('  ')[0]: regLineCheck = True
        
        if regLineCheck:
            if line.split('  ')[1] == traceInput.split('  ')[1]: memLineCheck = True
        
        if memLineCheck:
            if line.split('  ')[2] == traceInput.split('  ')[2]: actLineCheck = True
        
        if actLineCheck:
            if line == traceInput: return 'TRACE INPUT CORRECT'
        
    if regLineCheck != True: return 'ERROR WITH REGISTRY'
    elif memLineCheck != True: return 'ERROR WITH MEMORY'
    else: return 'ERROR WITH ACTIONS'
...
def main(tinyTrace:Trace) -> None:
    ...
    print('The following inputs are for the starter trace (What you\'re given)')
    print('-------------------------------------------------------------------')
    ...
    inpTrace = input('Enter TINY configuration (Format: x x x x  xxxxxxxxxxxxxxx): ').replace(' ', '')
    inpTraceActions = input('Enter the final piece of the trace (format xxx x x x): ')
    
    inputLine = makeInputLine(inpTrace, inpTraceActions)
    
    # Trace Header
    print('I L F A  Memory----------  Action---')
    print('P I R C  0123456789ABCDEF  OPR & ? !')
    print('-------  ----------------  ---------')
    print(f'{inputLine} <-- YOUR INPUT')
    ...
    print('-------  ----------------  ---------')
    print(f'{checkTraceOutput(trace.output, inputLine)} <-- TRACE OUTPUT')
    print('-------  ----------------  ---------')
